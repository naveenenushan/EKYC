...
- (void)onDetectionComplete:(NSDictionary *)resultInfo
{
    ...
    NSString *livenessId = resultInfo[@"livenessId"];
    UIImage *livenessImage = resultInfo[@"img"];
    
       NSLog(@"livenessId %@", livenessId);
       
       for(NSString *key in [resultInfo allKeys]) {
           NSLog(@"resultInfo%@",[resultInfo objectForKey:key]);
       }
       
    [self.delegate getLivenessId:livenessId];
    [self.delegate getLivenessImage:livenessImage];
